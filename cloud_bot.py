...
def ensure_account_today_reset(user_id):
    """Đảm bảo accounts_today của user được reset nếu sang ngày mới."""
    uid = str(user_id)
    today = get_vn_today()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kiểm tra pending tài khoản chưa xử lý
    uid = str(user_id)
...
async def get_cloud_machine(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = str(user_id)
    update_user_activity(user_id, user.username, user.first_name, user.last_name)
    # Đảm bảo reset accounts_today nếu sang ngày mới
    ensure_account_today_reset(user_id)

    # Kiểm tra xem có pending tài khoản chưa xử lý không
    if uid in PENDING and isinstance(PENDING[uid], dict) and PENDING[uid].get("status") != "creating":
        ...
        if pending_data.get("trial_received", False):
            ...
            # Gửi riêng thông tin tài khoản, không lộ trên nhóm
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"🎉 Tài khoản UMO Cloud của bạn:\n\n"
