...
# Load dữ liệu — migration đảm bảo mọi key tồn tại
...
def _flush_users_now():
    with _IO_LOCK:
        try:
            ...
            logger.error(f"flush users err: {e}")
...
def _flush_groups_now():
    with _IO_LOCK:
        try:
            ...
            logger.error(f"flush groups err: {e}")

def _flush_pending_now():
    with _IO_LOCK:
        try:
            bin_save(PENDING_BIN_ID, PENDING)
        except Exception as e:
            logger.error(f"flush pending err: {e}")

def save_data(force=False):
    with _SAVE_LOCK:
        _SAVE_DIRTY["users"] = True
    if force:
        _flush_users_now()

def save_groups(force=False):
    with _SAVE_LOCK:
        _SAVE_DIRTY["groups"] = True
    if force:
        _flush_groups_now()

def save_pending(force=False):
    with _SAVE_LOCK:
        _SAVE_DIRTY["pending"] = True
    if force:
        _flush_pending_now()
...
def _flush_all_if_dirty():
    with _SAVE_LOCK:
        u = _SAVE_DIRTY["users"]
        g = _SAVE_DIRTY["groups"]
        p = _SAVE_DIRTY["pending"]
    ...
    for attempt in range(2):
        try:
            ...
            if has_private_started(user_id):
                logger.warning(f"Fallback accept {uid} (status={m.status})")
                return True
            return False
        except Exception as e:
            logger.error(f"is_member err {uid} (try {attempt+1}): {e}")
            if attempt < 1:
                await asyncio.sleep(0.6)
                continue
            if has_private_started(user_id):
...
def send_verification_code(email, cuid):
    # UMO đã bỏ captcha — captcha="" là đúng
    data = {
    ...
    _res = jres(r)
    if _res.get("code") != 0:
        raise Exception(f"send verification failed: code={_res.get('code')} msg={_res.get('msg')}")
...
def set_password_cloud(user_id, token, password, cuid):
    ...
    r = req("POST", "https://oem-core.willclouds.com/saas-api/cloud-client/user/set-member-password",
            data=b, headers=h, timeout=20)
    if jres(r).get("code") != 0:
        raise Exception("set password failed")
    ...
    async with lock:
        ...
        try:
            ...
            if "1002025003" in em or "邮件发送过于频繁" in em or "too frequent" in em.lower():
                ...
                um = "❌ Đăng nhập UMO Cloud thất bại. Vui lòng thử lại."
            elif "No OTP" in em:
                um = "❌ Chưa nhận được mã xác minh. Vui lòng thử lại."
            elif "set password failed" in em:
                um = "❌ Đặt mật khẩu thất bại. Vui lòng thử lại."
        ...
        try:
            ...
            if ok:
                ...
                if "not eligible" in msg or code == 1003000071:
                    ...
                    await safe_edit(q,
                        "⛔ Nick này không đủ điều kiện nhận máy.\nVui lòng tạo tài khoản mới.")
                    if uid in PENDING:
                        del PENDING[uid]; save_pending()
                    ...
                    await safe_edit(q,
                        ...
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("🔄 Thử lại",
                                callback_data=f"receive_trial_yes:{user_id}")]
                        ]))
                else:
                    await safe_edit(q,
...
async def kiemtra(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if by_user:
        ...
        buf += "Chưa có ai tạo acc hôm nay."
    if len(buf) + len(footer) > MAX_LEN:
        chunks.append(buf); buf = footer
    else:
        buf += footer
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if m.status == "restricted":
            info += f"can_send_messages: {getattr(m, 'can_send_messages', 'N/A')}\n"
    ...
    for uid, data in list(PENDING.items()):
        ...
        if isinstance(data, dict) and data.get("status") != "creating":
            ...
            try:
                ...
                try:
                    await app.bot.send_message(chat_id=int(uid),
                        text="❌ Không thể tạo bù tài khoản. Vui lòng thử lại sau.")
                except Exception: pass
...
async def cleanup_spam_task():
    while True:
        ...
        for uid, info in list(SPAM_DATA.items()):
            ...
            if bu is None and now - ws > 120:
                expired.append(uid)
            elif bu is not None and now >= bu:
                expired.append(uid)
        for uid in expired:
