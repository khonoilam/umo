...
def github_save(payload):
    ...
    try:
        ...
        if r.status_code in [200, 201]:
            ...
            logger.warning(f"github_save status={r.status_code}")
    except Exception as e:
...
# ==================================================================
# KIỂM TRA THÀNH VIÊN — luôn verify với Telegram
# ==================================================================
async def is_member(context, user_id):
    ...
    for attempt in range(2):
        try:
            ...
            return uid in GROUP_MEMBERS_CACHE
    return False
...
# MAIL PROVIDERS
# ==================================================================
MAIL_PROVIDER_SCORE = {"mailtm": 100, "mailgw": 90, "tempmail_lol": 70, "1secmail": 50}
...
def _mail_provider_by_name(name):
    return {
        "mailtm": _mail_provider_mailtm,
        "mailgw": _mail_provider_mailgw,
        "1secmail": _mail_provider_1secmail,
        "tempmail_lol": _mail_provider_tempmail_lol,
    }.get(name)

def _provider_name_from_base(base):
    if "mail.tm" in base: return "mailtm"
    if "mail.gw" in base: return "mailgw"
    if "1sec" in base: return "1secmail"
    if "tempmail.lol" in base: return "tempmail_lol"
    return None

def create_temp_mail():
    sorted_names = sorted(MAIL_PROVIDER_SCORE.keys(),
                          key=lambda k: MAIL_PROVIDER_SCORE[k], reverse=True)
    last_err = None
    for name in sorted_names:
        fn = _mail_provider_by_name(name)
        if not fn: continue
        try:
            result = fn()
            logger.info(f"mail provider OK: {name}")
            return result
        except Exception as e:
            last_err = e
            logger.warning(f"mail provider {name} err: {e}")
            time.sleep(0.3)
...
async def read_code_from_mail_async(loop, token, timeout=45, base_url="https://api.mail.tm", status_cb=None):
    start = time.time()
    last_update = 0
...
# ==================================================================
# SEND ACC INFO — với highlight rõ ràng
# ==================================================================
async def send_acc_info(context, user_id, email, password, is_received=False, is_cancelled=False):
    if is_received:
        status = ("━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        status = ("━━━━━━━━━━━━━━━━━━━━━\n"
                  "❌ *ĐÃ HỦY LẤY MÁY TRIAL*\n"
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  "Bạn vẫn có thể đăng nhập UMO Cloud bằng tài khoản trên.")
    else:
        status = ("━━━━━━━━━━━━━━━━━━━━━\n"
                  "⚠️ *Bạn có muốn lấy máy sẵn không?*")
    text = (f"🎉 Tài khoản UMO Cloud của bạn:\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"📧 Email: `{email}`\n"
            f"🔑 Mật khẩu: `{password}`\n"
            f"{status}")
    markup = None if (is_received or is_cancelled) else trial_kb(user_id)
    try:
        await context.bot.send_message(chat_id=user_id, text=text, reply_markup=markup, parse_mode="Markdown")
        return True
    except Exception as e:
        try:
            ...
            logger.error(f"Send acc err: {e2}"); return False

# ==================================================================
# CHAT MEMBER — xóa private_started khi user rời nhóm
# ==================================================================
...
async def chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_s in ["left", "kicked"] and old_s not in ["left", "kicked"]:
        ...
        if uid in DATA.get("private_started", {}):
            del DATA["private_started"][uid]; save_data(force=True)
...
# ==================================================================
# CREATE ACC — với provider scoring
# ==================================================================
        ...
        try:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    otp_timeout = 45 if attempt == 0 else 60
                    code = await read_code_from_mail_async(loop, mtoken, otp_timeout, mbase, _status_cb)
                    logger.info(f"[{uid}] OTP received: {code}")
                    # Tăng điểm provider thành công
                    pname = _provider_name_from_base(mbase)
                    if pname:
                        MAIL_PROVIDER_SCORE[pname] = MAIL_PROVIDER_SCORE.get(pname, 50) + 5
                    break
                except Exception as e:
                    last_err = e
                    ...
                    if mbase:
                        pname = _provider_name_from_base(mbase)
                        if pname:
                            MAIL_PROVIDER_SCORE[pname] = max(0, MAIL_PROVIDER_SCORE.get(pname, 50) - 20)
            ...
            if "1002025003" in em or "邮件发送过于频繁" in em or "too frequent" in em.lower():
                ...
                um = "❌ Chưa nhận được mã xác minh sau nhiều lần thử. Vui lòng thử lại sau ít phút."
...
async def receive_trial_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        try:
            ...
            if ok:
                await safe_edit(q, "━━━━━━━━━━━━━━━━━━━━━\n✅ *ĐÃ LẤY MÁY TRIAL THÀNH CÔNG*\n━━━━━━━━━━━━━━━━━━━━━\n\nBạn có thể đăng nhập vào UMO Cloud bằng tài khoản đã gửi.\nNếu cần hỗ trợ, ib @jdaydichs", parse_mode="Markdown")
...
async def receive_trial_no(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        if pd and not isinstance(pd, bool) and pd.get("status") != "creating":
            try:
                await send_acc_info(context, user_id, pd['email'], pd['password'], is_cancelled=True)
            except Exception: pass
        if uid in PENDING:
            del PENDING[uid]; save_pending()
    await safe_edit(q,
        ...
        "❌ *ĐÃ HỦY LẤY MÁY TRIAL*\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
        "Tài khoản UMO Cloud của bạn đã được gửi trong tin nhắn riêng.\n"
        "Bạn vẫn có thể đăng nhập bằng tài khoản đó.\n\n"
    ...
    for uid, data in list(PENDING.items()):
        ...
        if isinstance(data, dict) and data.get("status") != "creating":
            ...
            try:
                ...
                cuid = str(uuid.uuid4()).replace("-", "")
                await loop.run_in_executor(EXECUTOR, send_verification_code, email, cuid)
                code = await read_code_from_mail_async(loop, mtoken, 45, mbase, None)
                cid, ctok = await loop.run_in_executor(EXECUTOR, login_email_code, email, code, cuid)
                npw = "aot" + ''.join(random.choices(string.ascii_letters + string.digits, k=5))
                await loop.run_in_executor(EXECUTOR, set_password_cloud, cid, ctok, npw, cuid)
                PENDING[uid] = {"email": email, "password": npw, "cloud_user_id": cid,
