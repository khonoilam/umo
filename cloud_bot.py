...
MAX_LEN = 3500  # Giới hạn an toàn (tính theo UTF-16)

HEADERS = {"X-Master-Key": JSONBIN_API_KEY, "Content-Type": "application/json"}

def utf16_len(s):
    """Đếm độ dài UTF-16 code units — đúng theo cách Telegram đếm."""
    ...
    for attempt in range(3):
        try:
            ...
            if has_private_started(user_id):
                logger.warning(f"Fallback accept {uid} (status={member.status})")
                return True
            return False
...
async def send_long(context, chat_id, text):
    """Chia nhỏ tin nhắn dài, đếm theo UTF-16 (cách Telegram đếm)."""
    ...
    chunk = ""
    for line in lines:
        if utf16_len(chunk) + utf16_len(line) + 1 > MAX_LEN:
            try:
...
async def chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status in ["member", "administrator", "creator", "restricted"] and old_status in ["left", "kicked"]:
        GROUP_MEMBERS_CACHE.add(uid)
        logger.info(f"User {uid} joined, added to cache")
...
async def notify_all_groups(context, text, tag_text=None):
    """Gửi thông báo tới các nhóm. Chỉ tag ở nhóm chính để đảm bảo notification."""
    groups = list(set(GROUPS))
    if GROUP_ID not in groups:
        groups.append(GROUP_ID)
    for gid in groups:
        try:
            if gid == GROUP_ID and tag_text:
                # Chỉ tag ở nhóm chính
                full = text + "\n\n" + tag_text
                await context.bot.send_message(chat_id=gid, text=full, parse_mode='HTML')
            else:
        ...
        try:
            ...
            if ok:
                ...
                if LAST_CLOUD_STATUS != "available":
                    ...
                    await notify_all_groups(context, txt, tag_text=tags if tags else None)
...
async def thongtin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, info in sorted_users:
        ...
        entry = f"👤 {display}\n   🆔 ID: {uid}\n   📦 Tổng acc: {ta}\n\n"
        if utf16_len(current) + utf16_len(entry) > MAX_LEN:
            chunks.append(current); current = entry
        else:
            current += entry
    if utf16_len(current) + utf16_len(footer) > MAX_LEN:
