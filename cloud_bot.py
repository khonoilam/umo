...
# CẤU HÌNH
# ==================================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8988947106:AAE0OdyRgF4i-a5Amu8TGjc9Ksntfj8KXhM")
ADMIN_ID = 7267437767
...
if isinstance(_cached.get("users"), dict):
    DATA = _cached["users"]
...
if isinstance(_cached.get("groups"), list):
    GROUPS = _cached["groups"]
if isinstance(_cached.get("pending"), dict):
    PENDING = _cached["pending"]

# Load JSONBin (overwrite nếu có data)
...
if isinstance(_tmp_g, list) and _tmp_g:
    GROUPS = _tmp_g
...
async def handle_pending_on_startup(app):
    ...
    for uid, data in list(PENDING.items()):
        ...
        if isinstance(data, dict) and data.get("status") != "creating":
            ...
            save_pending()
            try:
                await app.bot.send_message(chat_id=uid_int, text="⏳ Bot đang tạo bù tài khoản cho bạn...")
            ...
            try:
                ...
                save_pending()
                sent = await send_acc_info(app, uid_int, email, npw)
                if sent:
                    PENDING[uid]["account_sent"] = True
                    save_pending()
                try:
                    increment_user_account(uid_int)
                    LAST_ACCOUNT_CREATED[uid] = time.time()
                except Exception: pass
            except Exception as e:
...
def main():
    ...
    # FIX Python 3.12+: tạo event loop TRƯỚC khi build Application
    try:
