        ...
        if has_bot_mention:
            ...
            if can_delete:
                try:
                    ...
                    pass
            if user.id not in TAG_WARNED_USERS:
                try:
...
async def create_account_with_progress(context, query, user_id, user_mention):
    ...
    async with user_lock:
        ...
        try:
            ...
            # Cập nhật số lượng và gửi remaining (nếu lỗi cũng không ảnh hưởng)
            try:
...
def build_tag_mentions():
    ...
    for uid in DATA.get("tag_users", []):
        ...
        if username:
            ...
            tags.append(f"[{first_name}](tg://user?id={uid})")
    return " ".join(tags)
