...
# CẤU HÌNH
# ==================================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8988947106:AAFEkLGRLPquFzCgEV19iKWs2sK1EWjSCTI")
ADMIN_ID = 7267437767
...
def bin_load(bin_id):
    ...
    for attempt in range(3):
        try:
            r = requests.get(url, headers=BIN_HEADERS, timeout=30)
...
def bin_save(bin_id, data):
    ...
    for attempt in range(3):
        try:
            r = requests.put(url, headers=BIN_HEADERS, json=data, timeout=30)
            if r.status_code == 200:
                return True
        except Exception:
            pass
...
if isinstance(_tmp, dict):
    DATA = _tmp
    LAST_CLOUD_STATUS = _tmp.get("last_cloud_status")
...
async def is_member(context, user_id):
    """Kiểm tra user có trong nhóm chính không.
    - Cache nếu đã xác nhận.
    - Retry 3 lần nếu Telegram trả về left/kicked.
    - Chấp nhận status restricted.
    - Fallback: nếu user đã start bot → tạm chấp nhận.
    """
    uid = str(user_id)
...
def send_verification_code(email, cuid):
    ...
    if jres(r).get("code") != 0:
        raise Exception("send verification failed")
