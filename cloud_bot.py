...
# ==================================================================
# CLOUDFLARE WORKER PROXY (bypass 403)
# ==================================================================
...
def req(method, url, **kwargs):
    """Request qua Cloudflare Worker nếu có."""
    final_url = _route_through_worker(url)
    if final_url != url:
        logger.info(f"[REQ] routed via worker: {final_url[:100]}")

    last_exc = None
    for attempt in range(3):
        try:
            r = requests.request(method, final_url, timeout=20, **kwargs)
