...
# CẤU HÌNH
# ==================================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8988947106:AAHFY43SQLRP7Fkcgo2-a4x5rR3HyyDnhTw")
ADMIN_ID = 7267437767
...
if isinstance(_tmp, dict):
    ...
    DATA.setdefault("private_started", {})
    DATA.setdefault("banned", {})
    DATA.setdefault("tag_users", [])
    DATA.setdefault("last_cloud_status", None)
    LAST_CLOUD_STATUS = DATA.get("last_cloud_status")

_tmp_g = bin_load(GROUPS_BIN_ID)
if isinstance(_tmp_g, list):
    GROUPS = _tmp_g

_tmp_p = bin_load(PENDING_BIN_ID)
if isinstance(_tmp_p, dict):
    PENDING = _tmp_p

if GROUP_ID not in GROUPS:
    GROUPS.append(GROUP_ID)
    bin_save(GROUPS_BIN_ID, GROUPS)

# ==================================================================
# SAVE SYSTEM — non-blocking
# ==================================================================
...
def _flush_users_now():
    with _IO_LOCK:
        try: bin_save(USERS_BIN_ID, DATA)
        except Exception as e: logger.error(f"flush users err: {e}")
...
def _flush_pending_now():
    with _IO_LOCK:
        try: bin_save(PENDING_BIN_ID, PENDING)
        except Exception as e: logger.error(f"flush pending err: {e}")
...
def set_private_started(user_id):
    uid = str(user_id)
    if "private_started" not in DATA:
        DATA["private_started"] = {}
    if DATA["private_started"].get(uid):
        return
...
async def is_member(context, user_id):
    ...
    for attempt in range(2):
        try:
            ...
            logger.warning(f"is_member err {uid} (try {attempt+1}): {e}")
            if attempt < 1:
                await asyncio.sleep(0.6)
                continue
            if has_private_started(user_id):
...
def make_headers(token="", content_type=False):
    """Full browser headers để bypass WAF/Cloudflare."""
    h = {
...
def read_code_from_mail(token, timeout=240, base_url="https://api.mail.tm"):
    start = time.time()
...
def req(method, url, **kwargs):
    """Request với logging + proxy + retry thông minh."""
    if HTTP_PROXY or HTTPS_PROXY:
        kwargs.setdefault("proxies", {})
        if HTTP_PROXY: kwargs["proxies"]["http"] = HTTP_PROXY
        if HTTPS_PROXY: kwargs["proxies"]["https"] = HTTPS_PROXY
    ...
    for attempt in range(3):
        try:
            ...
            last_exc = e
            logger.warning(f"[REQ] {method} {url} err (attempt {attempt+1}): {type(e).__name__}: {e}")
            if attempt == 2:
                raise Exception(f"Network failed after 3 attempts: {type(last_exc).__name__}: {last_exc}")
            time.sleep(2)

def jres(r):
    """Parse JSON, log raw body nếu fail."""
    if r is None:
        logger.error("jres called with None response")
    ...
    try:
        ...
        return {"code": -1, "_status": r.status_code, "_body": body[:200]}

def send_verification_code(email, cuid):
    data = {
        "cuid": cuid, "ts": str(int(time.time()*1000)), "userId": "",
    ...
    r = req("POST", "https://oem-api.willclouds.com/saas-api/cloud-client/auth/send-verification-code",
            data=b, headers=h, timeout=20)
    _res = jres(r)
    if _res.get("code") != 0:
        raise Exception(
            f"send verification failed: "
            f"code={_res.get('code')} msg={_res.get('msg')} "
            f"status={_res.get('_status', r.status_code if r else 'None')} "
            f"body={_res.get('_body', '')[:200]}"
        )
...
def login_email_code(email, code, cuid):
    ...
    sig, b = sign(data)
    h = make_headers(content_type=True); h["x-signature"] = sig
    r = req("POST", "https://oem-core.willclouds.com/saas-api/cloud-client/auth/login",
            data=b, headers=h, timeout=20)
    res = jres(r)
    if res.get("code") != 0:
        raise Exception(f"login failed: code={res.get('code')} msg={res.get('msg')}")
    return res["data"]["userId"], res["data"]["token"]

def set_password_cloud(user_id, token, password, cuid):
    enc = rsa_encrypt(password)
    data = {
        "cuid": cuid, "ts": str(int(time.time()*1000)), "userId": str(user_id),
        "cid": CID, "chnl": CHANNEL, "cver": CVER, "locale": LOCALE,
        "clientType": CLIENT_TYPE, "password": enc
    }
    sig, b = sign(data)
    h = make_headers(token=token, content_type=True); h["x-signature"] = sig
    r = req("POST", "https://oem-core.willclouds.com/saas-api/cloud-client/user/set-member-password",
            data=b, headers=h, timeout=20)
    res = jres(r)
    if res.get("code") != 0:
        raise Exception(f"set password failed: code={res.get('code')} msg={res.get('msg')}")
...
def receive_trial_cloud(user_id, token, cuid):
    ...
    sig, b = sign(data)
    h = make_headers(token=token, content_type=True); h["x-signature"] = sig
    r = req("POST", "https://oem-api.willclouds.com/saas-api/cloud-client/user/receive-instance",
            data=b, headers=h, timeout=20)
    res = jres(r)
...
def login_password_cloud(email, password, cuid):
    ...
    data = {
        ...
        "cid": CID, "chnl": CHANNEL, "cver": CVER, "locale": LOCALE,
        "clientType": CLIENT_TYPE, "account": email,
        "loginType": "ACCOUNT_PASSWORD", "authContent": enc,
        "captcha": "", "p": password
    }
    sig, b = sign(data)
    h = make_headers(content_type=True); h["x-signature"] = sig
    ...
    if res.get("code") == 0:
        return res["data"]["userId"], res["data"]["token"]
    raise Exception(f"relogin failed: code={res.get('code')} msg={res.get('msg')}")
...
def check_login_cloud(user_id, token, cuid):
    ...
    sig, b = sign(data)
    h = make_headers(token=token, content_type=True); h["x-signature"] = sig
    r = req("POST", "https://oem-api.willclouds.com/saas-api/cloud-client/user/get-experience-qual",
            data=b, headers=h, timeout=20)
    return jres(r)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pd and isinstance(pd, dict) and pd.get("status") != "creating":
        if update.effective_chat.type == "private":
            ...
            await update.message.reply_text(
                "⚠️ Bạn đang có tài khoản chưa nhận máy. Vui lòng kiểm tra tin nhắn riêng với bot."
            )
        return
    ...
    if update.effective_chat.type == "private":
        ...
        if not is_mem:
            await update.message.reply_text(
                ...
                f"👉 {GROUP_LINK}\n\n"
                f"Sau khi tham gia, bấm nút Verify bên dưới.",
...
async def create_acc_progress(context, query, user_id, user_mention):
    ...
    async with lock:
        ...
        if now < GLOBAL_RATE_LIMIT_UNTIL:
            ...
            await safe_edit(query, f"⏳ Server đang bận, vui lòng chờ {wait}s rồi thử lại.")
            return
        last = USER_COOLDOWN.get(uid, 0)
        if now - last < COOLDOWN_SECONDS:
            wait = int(COOLDOWN_SECONDS - (now - last))
            await safe_edit(query, f"⏳ Bạn vừa tạo acc, chờ {wait}s nữa rồi thử lại.")
            return
        ...
        try:
            ...
            code = await loop.run_in_executor(EXECUTOR, read_code_from_mail, mtoken, 240, mbase)
            ...
            PENDING[uid] = {
                "email": email, "password": npw,
                "cloud_user_id": cid, "cloud_token": ctok, "cuid": cuid,
            ...
            try:
                ...
                if user_id != ADMIN_ID:
                    ...
                    rem = max(0, 4 - at)
                    await context.bot.send_message(chat_id=user_id,
                        text=f"📊 Hôm nay bạn còn {rem}/4 lượt tạo tài khoản.")
            except Exception as e:
            ...
            if "1002025003" in em or "邮件发送过于频繁" in em or "too frequent" in em.lower():
                ...
                um = "❌ Chưa nhận được mã xác minh. Vui lòng thử lại."
...
async def get_cloud_machine(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_member(context, user_id):
        await safe_edit(q, "❌ Bạn chưa tham gia nhóm! Vui lòng tham gia rồi bấm Verify.",
    ...
    async with lock:
        ...
        try:
            ...
            if chk.get("code") == 1000000008:
                ...
                nu, nt = await loop.run_in_executor(
                    EXECUTOR, login_password_cloud, pd["email"], pd["password"], cuid)
                pd["cloud_user_id"] = nu
                pd["cloud_token"] = nt
                save_pending()
                cid, ctok = nu, nt
            ok, res = await loop.run_in_executor(
                EXECUTOR, lambda: receive_trial_cloud(cid, ctok, cuid))
            if ok:
                await safe_edit(q,
...
async def thongtin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, info in sorted_users:
        ...
        entry = f"👤 {display}\n   🆔 ID: {uid}\n   📦 Tổng acc: {ta}\n\n"
        if len(buf) + len(entry) > MAX_LEN:
            chunks.append(buf); buf = entry
        else: buf += entry
    if len(buf) + len(footer) > MAX_LEN:
        chunks.append(buf); buf = footer
    else: buf += footer
    ...
    for uid, data in list(PENDING.items()):
        ...
        if isinstance(data, dict) and data.get("status") != "creating":
            ...
            try:
                ...
                logger.error(f"Pending recovery err for {uid}: {e}")
                try:
                    await app.bot.send_message(chat_id=int(uid),
                        text=f"❌ Không thể tạo bù tài khoản. Vui lòng thử lại sau.")
                except Exception: pass
...
async def backup_task():
    ...
    while True:
        ...
        try: await loop.run_in_executor(EXECUTOR, _flush_all_if_dirty)
