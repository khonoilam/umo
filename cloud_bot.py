...
async def chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Xử lý khi bot được thêm vào nhóm hoặc khi thành viên rời nhóm."""
    my_chat_member = update.my_chat_member
    if my_chat_member and my_chat_member.new_chat_member.user.id == context.bot.id:
    ...
    for uid, data in list(PENDING.items()):
        if isinstance(data, dict) and data.get("status") != "creating":
            ...
            try:
                ...
                # Cập nhật số lượng (bọc riêng để tránh lỗi ảnh hưởng)
                try:
...
def main():
    ...
    # Handler cho sự kiện thành viên rời nhóm chính
