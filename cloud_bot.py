...
if isinstance(_cached.get("users"), dict):
    DATA = _cached["users"]
    for k in ["users", "daily_counts", "private_started", "banned", "tag_users"]:
        DATA.setdefault(k, {} if k != "tag_users" else [])
...
def _flush_all_if_dirty():
    with _SAVE_LOCK:
        ...
        _SAVE_DIRTY["users"] = _SAVE_DIRTY["groups"] = _SAVE_DIRTY["pending"] = False
    _save_local_cache()
...
def make_headers(token="", content_type=False):
    # KHÔNG set Accept-Encoding — để requests tự xử lý gzip
    h = {
...
def _mail_provider_tempmail_lol():
    ...
    if not email or not token: raise Exception(f"tempmail.lol bad response")
